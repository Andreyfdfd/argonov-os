...
*16915 байт · 478 строк*

```bash
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# ═══════════════════════════════════════════════════════
#  ARGONOV OS · Main Entry v2.1
#  Единая точка входа — читает реестр скриптов
#  Версия: 2.1  ·  Обновлён: 2026-09-11
# ═══════════════════════════════════════════════════════
"""
...
Git / backup:
    argonov push · pull · status · backup · restore · doctor
...
# ═══ СПРАВКА (через таблицы — без переносов) ═══
def show_help():
    header()
    ...
    t.add_column("", style=f"bold {YELLOW}", width=14)
    t.add_column("", style=CYAN, width=30)
    t.add_column("", style=f"bold {YELLOW}", width=14)
    t.add_column("", style=CYAN, width=30)
    t.add_row("🧠 ai",      "AI-ассистент",   "📈 crypto", "Крипта и валюты")
    t.add_row("🎵 music",   "Музыкальный плеер", "📌 todo", "Задачи")
    t.add_row("📝 notes",   "Заметки",         "🔒 pm",     "Пароли (AES-256)")
    t.add_row("🎮 hack",    "OSINT мультитул", "🕹  rpg",   "RPG-симулятор")
    t.add_row("📥 d",       "Загрузчик",       "🌧  m",     "Матрица")
    t.add_row("🔐 passgen", "Генератор паролей","⚡ s",     "Sysinfo")
    t.add_row("📦 util",    "Сканер пакетов",  "🎨 art",    "Картинки")
    t.add_row("🩺 doctor",  "Диагностика",     "",          "")
    console.print(t)
    console.print()
    ...
    t.add_column("", style=f"bold {YELLOW}", width=14)
    t.add_column("", style=CYAN, width=30)
    t.add_row("push",    "git push",      "pull",    "git pull")
    t.add_row("status",  "git status",    "backup",  "архивный бэкап")
    t.add_row("restore", "из бэкапа",     "doctor",  "диагностика")
    console.print(t)
    console.print(f"  [{GRAY}]doctor --full — с проверкой сети[/]")
    console.print()

    # РЕЕСТР
    console.print(f"[bold {MAGENTA}][ РЕЕСТР СКРИПТОВ ][/]")
    console.print(f"[{GREEN_DIM}]{'═' * 55}[/]")
    console.print()
    t = Table(box=None, show_header=False, padding=(0, 2), expand=True)
    t.add_column("", style=f"bold {YELLOW}", width=38)
    t.add_column("", style=CYAN, width=30)
    t.add_row("argonov-core list",         "все скрипты")
    t.add_row("argonov-core info <name>",  "инфо о скрипте")
    t.add_row("argonov-core edit <name>",  "открыть в nano")
    t.add_row("argonov-core run <name>",   "запустить")
    t.add_row("argonov-core check",        "проверить всё")
    console.print(t)
    console.print()
...
def do_backup():
    ...
    all_items = " ".join(f'"{x}"' for x in script_files + extras
